...
class SearchArgs(BaseModel):
    ...
    rff_k: int = Field(default=60, ge=0, le=16384, description="Smoothing constant for Reciprocal Rank Fusion (RRF) algorithm in hybrid search (recommended: from 10 to 100).")  # fmt: off
    ...
    score_threshold: float = Field(default=0.0, ge=0.0, le=1.0, description="Score of cosine similarity threshold for filtering results, only available for semantic search method.")  # fmt: off

    @model_validator(mode="after")
    def score_threshold_filter(cls, values):
        if values.score_threshold > 0.0 and values.method != SearchMethod.SEMANTIC:
