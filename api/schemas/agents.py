...
from api.schemas.chat import ChatSearchArgs
...
class AgentsChatCompletionRequest(BaseModel):
    ...
    tools: list[dict[str, Any]] | None = Field(default=[{"type": "all"}], description="A list of tools the model may call. Get the list of tools with the `/v1/tools` endpoint. Tools must be provide as dict `{'type': 'tool_name'}`.")  # fmt: off
    ...
    search_args: ChatSearchArgs | None = Field(default=None, deprecated=True)  # fmt: off
